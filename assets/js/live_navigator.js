...
export function initNavigator(connectOptions, options) {
  if (connectOptions === undefined) connectOptions = {};
  ...
  if (typeof tabNum === 'string') tabNum = parseInt(tabNum);
  if (isNaN(tabNum) || typeof tabNum !== 'number') tabNum = undefined;
  if (!tabNum) {
    tabNum = localStorage.getItem(key);
    if (typeof tabNum === 'string') tabNum = parseInt(tabNum);
    if (isNaN(tabNum) || typeof tabNum !== 'number') tabNum = 1;
    localStorage.setItem(key, tabNum + 1);
    sessionStorage.setItem(key, tabNum);
  }
  if (!connectOptions.params) connectOptions.params = {};
  connectOptions.params[navigatorTabParam] = tabNum;

  window.addEventListener(`phx:${addPrefix('set-tab', navigatorEvents)}`, event => {
    const { tab } = event.detail;
    if (typeof tab !== 'number' || isNaN(tab) || tab < 1) return;
    sessionStorage.setItem(key, tabNum);
  });
